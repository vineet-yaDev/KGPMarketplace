...
import { Search, Grid, List, X, Filter, IndianRupee } from 'lucide-react'
...
const halls = [
  'RK', 'RP', 'MS', 'LLR', 'MMM', 'LBS',
  'AZAD', 'PATEL', 'NEHRU', 'SNIG', 'SNVH', 'MT'
]

const productTypes = [
  'NEW', 'USED', 'RENT', 'SERVICE'
]
...
const seasonalities = [
  'NONE', 'HALL_DAYS', 'PLACEMENTS', 'SEMESTER_END', 'FRESHERS', 'FESTIVE'
]
...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    if (category && productCategories.includes(category.toUpperCase())) {
      setSelectedCategory(category.toUpperCase())
    }
    if (hall && halls.includes(hall.toUpperCase())) {
      setSelectedHall(hall.toUpperCase())
    }
    if (type && productTypes.includes(type.toUpperCase())) {
      setSelectedType(type.toUpperCase())
    }
    if (status && productStatuses.includes(status.toUpperCase())) {
      setSelectedStatus(status.toUpperCase())
    }
    if (seasonality && seasonalities.includes(seasonality.toUpperCase())) {
...
const FilterDropdown = ({ label, value, onValueChange, options, placeholder }: any) => (
  <div className="mb-4">
      ...
      <SelectContent className="glass">
        ...
        {options.map((option: any) => (
          <SelectItem key={option.value || option} value={option.value || option}>
            {option.label || formatEnumName(option)}
          </SelectItem>
        ))}
      </SelectContent>
    </Select>
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex gap-8">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="glass-card p-4 mb-6">
                ...
                {hasActiveFilters && (
                  <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-white/10">
                    ...
                    {selectedSeasonality && (
                      <Badge variant="secondary" className="flex items-center gap-1">
                        {formatEnumName(selectedSeasonality)}
                        <X 
                          className="w-3 h-3 cursor-pointer hover:text-destructive" 
              ...
              {/* Products Grid/List - Same as before */}
              {viewMode === 'grid' ? (
                      ...
                      <Card className="glass-card hover-lift overflow-hidden h-full">
                        <div className="aspect-video relative overflow-hidden">
                          ...
                          <Badge className="absolute top-2 left-2 bg-primary text-white">
                            {formatEnumName(product.productType || 'USED')}
                          </Badge>
                        </div>
